...
void loop() {
  ...
  if (StartPlayTransition == true) {  //code to be executed until transition completed ===========================
    //get array from RaspberyPi (ALWAYS?: no stand alone operation possible....,but how to solve this elegantly? (timeout?)
...
void receiveCharacter() {
  ...
  if (inChar != -1) {
    ...
    if (inChar == '\n') {
      ...
      if(readString.equals("-32001\n")){ //not receiving but sending an array from arduino to pc
        receiveArrayFromPC = false;
      }
      ReadNr = ReadNr + 1;
      readString = "";
